...
$addItem = function () {
    $this->items[] = ['id' => null, 'inventory_item_id' => '', 'quantity' => 1, 'unit_price' => 0];
};
...
$save = function () {
    ...
    $purchase->update(['supplier_id' => $this->supplier_id, 'total_amount' => $total]);
...
<div>
    <div class="p-6 max-w-3xl">
        <x-admin.page-header
            ...
            :breadcrumbs="[['label'=>'Admin','url'=>route('admin.dashboard')],['label'=>'Purchases','url'=>route('admin.purchases.index')],['label'=>'Edit']]"
        />
            ...
            <form wire:submit="save" class="space-y-6">
                ...
                <div>
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <button type="button" wire:click="addItem" class="text-xs text-orange-600 hover:text-orange-700 font-medium flex items-center gap-1">
                            ...
                            <div class="grid grid-cols-12 gap-2 items-start" wire:key="item-{{ $index }}">
                                <div class="col-span-5">
                                    <select wire:model="items.{{ $index }}.inventory_item_id"
                                        class="w-full px-2 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500">
                                ...
                                <div class="col-span-3">
                                    <input type="number" wire:model="items.{{ $index }}.quantity" step="0.01" min="0.01" placeholder="Qty"
                                        class="w-full px-2 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500">
                                </div>
                                <div class="col-span-3">
                                    <input type="number" wire:model="items.{{ $index }}.unit_price" step="0.01" min="0" placeholder="Unit $"
                                        class="w-full px-2 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500">
                                </div>
                ...
                <div class="flex items-center gap-3 pt-2">
                    <x-admin.btn type="submit" wire:loading.attr="disabled">
                        <span wire:loading.remove>Update Purchase</span>
